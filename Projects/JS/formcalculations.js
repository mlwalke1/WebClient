...
{
    var cakeLayerPrice=0;
    //Get a reference to the form id="cakeform"
    var theForm = document.forms["cakeform"];
    //Get a reference to the select id="layers"
     var selectedLayer = theForm.elements["layers"];
     
    //set cakeFilling Price equal to value user chose
    //For example filling_prices["Lemon".value] would be equal to 5
    cakeLayerPrice = layer_prices[selectedLayer.value];

    //finally we return cakeFillingPrice
    return cakeLayerPrice;
}
...
function calculateTotal()
{
    ...
    var cakePrice = getCakeSizePrice() + getFillingPrice() + candlesPrice() + insciptionPrice() +getLayerPrice();
